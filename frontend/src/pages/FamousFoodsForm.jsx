...
import { Plus, Minus, ArrowRight, ImagePlus, Video, Instagram,X,Loader2 } from "lucide-react";
import api from "@/api/axios"
import { State, City } from "country-state-city";
import { useAuth } from "../context/AuthProvider";

export default function FamousFoodsForm() {
  ...
  const { register, handleSubmit,setValue,watch, control, reset,formState: { errors,isSubmitting }, } = useForm({
    defaultValues: {
      ...
      category: "",
      description: "",
      places: [{ placeName: "", address: "", price: "", specialNote: "", state:"",city:"" }],
      images: [""],
      videoLink: "",
      instagramLink: "",
      permission: false,
...
const [selectedFiles, setSelectedFiles] = useState([]);
  ...
  const onSubmit = useCallback(async (data) => {
    ...
    try {
      ...
      // Save Food or Contribute
      const res = await api.post("/famous-food", {
        ...data,
        images: imageUrls,
      });
      ...
      const message = res.data.isNewPlace 
        ...
        : "Your contribution has been added to this food item!";
      alert(message);
      reset();
      setSelectedFiles([]); 
  ...
  return (
    <section className="relative pt-6 pb-14 sm:pt-8 sm:pb-20 bg-gradient-to-b from-yellow-50 via-amber-100 to-orange-50 overflow-hidden">
      ...
      <div className="relative container mx-auto px-6">
        ...
        >
          <h2 className="text-3xl sm:text-5xl font-bold mb-1 sm:mb-2 font-serif bg-gradient-to-r from-orange-500 via-amber-600 to-red-600 bg-clip-text text-transparent drop-shadow-sm">
            Add a Famous Food
          </h2>
          <p className="text-base sm:text-lg text-orange-700 font-medium">
            Share your city’s most iconic dishes and where to find them at their best
          </p>
        ...
        >
          <Card className="relative bg-white/70 backdrop-blur-xl border border-amber-200 rounded-3xl shadow-xl p-5 sm:p-8 max-w-3xl mx-auto space-y-4 sm:space-y-6">
            <form onSubmit={handleSubmit(onSubmit)} className="space-y-5 sm:space-y-8">
              {/* Basic Info */}
              <div>
                <label className="block text-amber-900 font-semibold mb-1 sm:mb-2">Food Name</label>
                <input
  ...
  <div>
    ...
    {errors.city && (
      <p className="text-red-500 text-sm mt-1">City is required</p>
              ...
              {/* Dynamic Places */}
              <div>
                <h3 className="text-xl font-bold text-orange-800 mb-4">Places Famous for This Food</h3>
                <div className="space-y-6">
                  {fields.map((field, index) => (
                    <div
                      key={field.id}
                      className="p-4 rounded-2xl border border-amber-200 bg-white/80 relative"
                    >
                      <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 sm:gap-4">
                        <input
                          {...register(`places.${index}.placeName`, { required: true })}
                          placeholder="Place Name"
                          className="p-2.5 sm:p-3 rounded-xl border border-amber-200 bg-white/80 focus:ring-2 focus:ring-amber-400 outline-none"
                        />
                        <input
                          {...register(`places.${index}.address`, { required: true })}
                        ...
                        />
                        <input
                          {...register(`places.${index}.specialNote`)}
                      ...
                      {fields.length > 1 && (
                        <Button
                          ...
                          className="absolute top-2 right-2 rounded-full bg-red-100 text-red-700 hover:bg-red-200"
                          size="icon"
                        >
                ...
                <Button
                  type="button"
                  onClick={() =>
                    append({ placeName: "", address: "", price: "", specialNote: "" })
                  }
                  className="mt-4 flex items-center gap-2 bg-amber-100 hover:bg-amber-200 text-orange-800 rounded-full"
                >
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 sm:gap-6">
                ...
                <div className="sm:col-span-2">
                  <label className="block text-amber-900 font-semibold mb-1 sm:mb-2 flex items-center gap-2">
                    <Instagram className="w-5 h-5 text-amber-600" /> Instagram Link (optional)
                  </label>
                  <input
                    type="url"
              ...
              <div className="flex items-start gap-3">
                ...
                <p className="text-sm text-amber-800">
                  I confirm that the information and media I’ve shared are accurate and I
                  allow <span className="font-semibold text-orange-700">SheherJaano</span> to
              ...
              >
                {isSubmitting ? (
                  ...
                  <>
                    Add Food <ArrowRight className="h-5 w-5 text-orange-800" />
                  </>
                )}
              </Button>
            </form>
