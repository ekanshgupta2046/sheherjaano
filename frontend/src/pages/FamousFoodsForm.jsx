import { useFieldArray, useForm } from "react-hook-form";
import { motion } from "framer-motion";
import { Card } from "@/components/ui/card";
import { Button } from "@/components/ui/button";
import { Plus, Minus, ArrowRight, ImagePlus, Video, Instagram } from "lucide-react";

export default function FamousFoodsForm() {
  const { register, handleSubmit, control, reset } = useForm({
    defaultValues: {
      foodName: "",
      category: "",
      description: "",
      places: [{ placeName: "", address: "", price: "", specialNote: "" }],
      foodImages: [],
      videoLink: "",
      instagramLink: "",
      permission: false,
    },
  });

  const { fields, append, remove } = useFieldArray({
    control,
    name: "places",
  });

  const onSubmit = (data) => {
    console.log("Food Form Data:", data);
    alert("Food submitted successfully!");
    reset();
  };

  return (
    <section className="relative pt-8 pb-20 bg-gradient-to-b from-yellow-50 via-amber-100 to-orange-50 overflow-hidden">
      <div className="absolute inset-0 bg-[url('/ethnic-pattern.png')] opacity-[0.04] bg-repeat"></div>

      <div className="relative container mx-auto px-6">
        {/* Header */}
        <motion.div
          initial={{ opacity: 0, y: 25 }}
          whileInView={{ opacity: 1, y: 0 }}
          transition={{ duration: 0.5 }}
          viewport={{ once: true }}
          className="text-center mb-12"
        >
          <h2 className="text-5xl font-bold mb-3 font-serif bg-gradient-to-r from-orange-500 via-amber-600 to-red-600 bg-clip-text text-transparent drop-shadow-sm">
            Add a Famous Food
          </h2>
          <p className="text-lg text-orange-700 font-medium">
            Share your city’s most iconic dishes and where to find them at their best
          </p>
          <div className="mt-5 w-24 h-1 mx-auto bg-gradient-to-r from-amber-400 to-yellow-300 rounded-full"></div>
        </motion.div>

        {/* Form Card */}
        <motion.div
          initial={{ opacity: 0, y: 25 }}
          whileInView={{ opacity: 1, y: 0 }}
          transition={{ duration: 0.6 }}
          viewport={{ once: true }}
        >
          <Card className="relative bg-white/70 backdrop-blur-xl border border-amber-200 rounded-3xl shadow-xl p-8 max-w-3xl mx-auto space-y-6">
            <form onSubmit={handleSubmit(onSubmit)} className="space-y-8">
              {/* Basic Info */}
              <div>
                <label className="block text-amber-900 font-semibold mb-2">Food Name</label>
                <input
                  {...register("foodName", { required: true })}
                  placeholder="e.g. Chole Bhature"
                  className="w-full p-3 rounded-xl border border-amber-200 bg-white/80 focus:ring-2 focus:ring-amber-400 outline-none text-amber-900"
                />
              </div>

              <div>
                <label className="block text-amber-900 font-semibold mb-2">Category</label>
                <select
                  {...register("category", { required: true })}
                  className="w-full p-3 rounded-xl border border-amber-200 bg-white/80 focus:ring-2 focus:ring-amber-400 outline-none text-amber-900"
                >
                  <option value="">Select category</option>
                  <option>Street Food</option>
                  <option>Sweet</option>
                  <option>Beverage</option>
                  <option>Snack</option>
                  <option>Main Course</option>
                </select>
              </div>

              <div>
                <label className="block text-amber-900 font-semibold mb-2">Description</label>
                <textarea
                  {...register("description")}
                  rows="4"
                  placeholder="Describe what makes this dish special..."
                  className="w-full p-3 rounded-xl border border-amber-200 bg-white/80 focus:ring-2 focus:ring-amber-400 outline-none text-amber-900"
                />
              </div>

              {/* Dynamic Places */}
              <div>
                <h3 className="text-xl font-bold text-orange-800 mb-4">Places Famous for This Food</h3>
                <div className="space-y-6">
                  {fields.map((field, index) => (
                    <div
                      key={field.id}
                      className="p-4 rounded-2xl border border-amber-200 bg-white/80 relative"
                    >
                      <div className="grid sm:grid-cols-2 gap-4">
                        <input
                          {...register(`places.${index}.placeName`, { required: true })}
                          placeholder="Place Name"
                          className="p-3 rounded-xl border border-amber-200 bg-white/80 focus:ring-2 focus:ring-amber-400 outline-none"
                        />
                        <input
                          {...register(`places.${index}.address`, { required: true })}
                          placeholder="Address / Area"
                          className="p-3 rounded-xl border border-amber-200 bg-white/80 focus:ring-2 focus:ring-amber-400 outline-none"
                        />
                        <input
                          {...register(`places.${index}.price`)}
                          placeholder="Price (₹)"
                          className="p-3 rounded-xl border border-amber-200 bg-white/80 focus:ring-2 focus:ring-amber-400 outline-none"
                        />
                        <input
                          {...register(`places.${index}.specialNote`)}
                          placeholder="Special note (optional)"
                          className="p-3 rounded-xl border border-amber-200 bg-white/80 focus:ring-2 focus:ring-amber-400 outline-none"
                        />
                      </div>
                      {fields.length > 1 && (
                        <Button
                          type="button"
                          onClick={() => remove(index)}
                          className="absolute top-2 right-2 rounded-full bg-red-100 text-red-700 hover:bg-red-200"
                          size="icon"
                        >
                          <Minus className="w-4 h-4" />
                        </Button>
                      )}
                    </div>
                  ))}
                </div>

                <Button
                  type="button"
                  onClick={() =>
                    append({ placeName: "", address: "", price: "", specialNote: "" })
                  }
                  className="mt-4 flex items-center gap-2 bg-amber-100 hover:bg-amber-200 text-orange-800 rounded-full"
                >
                  <Plus className="w-4 h-4" /> Add Another Place
                </Button>
              </div>

              {/* Media Upload */}
              <div className="grid sm:grid-cols-2 gap-6">
                <div>
                  <label className="block text-amber-900 font-semibold mb-2 flex items-center gap-2">
                    <ImagePlus className="w-5 h-5 text-amber-600" /> Upload Food Images
                  </label>
                  <input
                    type="file"
                    multiple
                    accept="image/*"
                    {...register("foodImages")}
                    className="w-full p-2 rounded-xl border border-amber-200 bg-white/80 focus:ring-2 focus:ring-amber-400 outline-none"
                  />
                </div>

                <div>
                  <label className="block text-amber-900 font-semibold mb-2 flex items-center gap-2">
                    <Video className="w-5 h-5 text-amber-600" /> Video Link (optional)
                  </label>
                  <input
                    type="url"
                    {...register("videoLink")}
                    placeholder="https://youtube.com/..."
                    className="w-full p-3 rounded-xl border border-amber-200 bg-white/80 focus:ring-2 focus:ring-amber-400 outline-none"
                  />
                </div>

                <div className="sm:col-span-2">
                  <label className="block text-amber-900 font-semibold mb-2 flex items-center gap-2">
                    <Instagram className="w-5 h-5 text-amber-600" /> Instagram Link (optional)
                  </label>
                  <input
                    type="url"
                    {...register("instagramLink")}
                    placeholder="https://instagram.com/..."
                    className="w-full p-3 rounded-xl border border-amber-200 bg-white/80 focus:ring-2 focus:ring-amber-400 outline-none"
                  />
                </div>
              </div>

              {/* Permission */}
              <div className="flex items-start gap-3">
                <input
                  type="checkbox"
                  {...register("permission", { required: true })}
                  className="mt-1 accent-amber-600"
                />
                <p className="text-sm text-amber-800">
                  I confirm that the information and media I’ve shared are accurate and I
                  allow <span className="font-semibold text-orange-700">SheherJaano</span> to
                  display them publicly.
                </p>
              </div>

              {/* Submit Button */}
              <Button
                type="submit"
                className="w-full flex items-center justify-center gap-2 py-3 font-bold text-orange-800
                  bg-gradient-to-r from-yellow-200 via-yellow-100 to-white
                  shadow-md hover:shadow-lg active:scale-95 transition-transform duration-200 rounded-full"
              >
                Add Food <ArrowRight className="h-5 w-5 text-orange-800" />
              </Button>
            </form>
          </Card>
        </motion.div>
      </div>
    </section>
  );
}
