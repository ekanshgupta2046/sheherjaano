"use client";

import { useEffect, useState } from "react";
import { useParams, useNavigate } from "react-router-dom";
import { motion } from "framer-motion";
import api from "@/api/axios";

import {
  ArrowLeft,
  MapPin,
  Star,
  ExternalLink,
  Youtube,
  Instagram,
  Share2,
  Heart,
} from "lucide-react";

import { Avatar, AvatarImage, AvatarFallback } from "@/components/ui/avatar";
import DynamicMap from "@/components/custom/DynamicMap";

export default function HandicraftDetails() {
  const { id } = useParams();
  const navigate = useNavigate();

  const [handicraft, setHandicraft] = useState(null);
  const [loading, setLoading] = useState(true);

  useEffect(() => {
    const fetchHandicraft = async () => {
      try {
        const res = await api.get(`/handicrafts/${id}`);
        setHandicraft(res.data.data);
      } catch (err) {
        console.error("Failed to fetch handicraft", err);
      } finally {
        setLoading(false);
      }
    };

    fetchHandicraft();
    window.scrollTo(0, 0);
  }, [id]);

  if (loading) {
    return (
      <div className="min-h-screen flex items-center justify-center bg-[#FFF7ED]">
        <div className="w-14 h-14 border-4 border-amber-300 border-t-amber-500 rounded-full animate-spin" />
      </div>
    );
  }

  if (!handicraft) {
    return (
      <div className="min-h-screen flex items-center justify-center text-xl sm:text-2xl font-serif text-amber-900">
        Handicraft not found.
      </div>
    );
  }

  /* ========= MAP COORDS ========= */
  const safeNumber = (v) => (Number.isFinite(Number(v)) ? Number(v) : null);

  const lat = safeNumber(handicraft?.geometry?.coordinates?.[1]);
  const lng = safeNumber(handicraft?.geometry?.coordinates?.[0]);
  const hasCoords = lat !== null && lng !== null;

  const primaryMarket = handicraft.localMarkets?.[0];

  const googleMapsUrl = hasCoords
    ? `https://www.google.com/maps/dir/?api=1&destination=${lat},${lng}`
    : `https://www.google.com/maps/dir/?api=1&destination=${encodeURIComponent(
        `${primaryMarket?.name || handicraft.name} ${handicraft.city} ${handicraft.state}`
      )}`;

  return (
    <div className="min-h-screen bg-gradient-to-b from-[#FFF7ED] via-[#FFF3D6] to-[#FFFBEA]">

      {/* ========================= HERO ========================= */}
      <div className="relative h-[55vh] sm:h-[65vh] lg:h-[75vh] overflow-hidden">

        <motion.button
          initial={{ opacity: 0, x: -20 }}
          animate={{ opacity: 1, x: 0 }}
          onClick={() => navigate(-1)}
          className="absolute top-4 left-4 sm:top-6 sm:left-6 z-30 bg-white/70 p-2 sm:p-3 rounded-full shadow-md hover:bg-white"
        >
          <ArrowLeft className="w-5 h-5 sm:w-6 sm:h-6 text-amber-800" />
        </motion.button>


        <img
          src={handicraft.images?.[0] || "/placeholder-image.jpg"}
          alt={handicraft.name}
          className="w-full h-full object-cover"
        />

        <div className="absolute inset-0 bg-gradient-to-t from-amber-900/70 via-amber-600/30 to-transparent" />

        <div className="absolute bottom-0 left-0 w-full p-6 sm:p-10 md:p-16 text-white">
          <h1 className="text-3xl sm:text-4xl md:text-5xl lg:text-6xl font-serif font-semibold leading-tight mb-3">
            {handicraft.name}
          </h1>

          <div className="flex flex-wrap gap-4 text-sm sm:text-lg text-amber-100">
            <div className="flex items-center gap-1">
              <MapPin className="w-4 sm:w-5 h-4 sm:h-5 text-yellow-300" />
              {handicraft.city}, {handicraft.state}
            </div>

            <div className="flex items-center gap-1 bg-white/20 px-3 py-1 rounded-lg">
              <Star className="w-4 sm:w-5 h-4 sm:h-5 text-yellow-300 fill-yellow-300" />
              <span>{handicraft.rating > 0 ? handicraft.rating.toFixed(1) : "New"}</span>
              <span className="text-xs opacity-75">
                ({handicraft.totalRatings ?? 0} reviews)
              </span>
            </div>
          </div>
        </div>
      </div>

      {/* ========================= CONTENT ========================= */}
      <div className="container mx-auto px-4 sm:px-6 py-12 sm:py-16 -mt-10 grid grid-cols-1 lg:grid-cols-3 gap-8 sm:gap-12">

        {/* LEFT COLUMN */}
        <div className="lg:col-span-2 space-y-8 sm:space-y-12">

          {/* ABOUT */}
          <section className="bg-white p-6 sm:p-10 rounded-2xl sm:rounded-[2.5rem] shadow-lg border border-amber-100">
            <h2 className="text-2xl sm:text-3xl font-serif font-semibold text-amber-900 mb-5 sm:mb-6">
              About the Handicraft
            </h2>

            <p className="text-gray-700 leading-relaxed text-base sm:text-lg whitespace-pre-line">
              {handicraft.description || "No description available."}
            </p>

            {(handicraft.videoLink || handicraft.instagramLink) && (
              <div className="mt-6 sm:mt-8 flex flex-wrap gap-3 sm:gap-4 pt-4 sm:pt-6 border-t border-amber-100">
                {handicraft.videoLink && (
                  <a
                    href={handicraft.videoLink}
                    target="_blank"
                    className="flex items-center gap-2 px-4 py-2 bg-red-50 text-red-700 rounded-lg hover:bg-red-100 text-sm"
                  >
                    <Youtube className="w-4 h-4" /> Watch Video
                  </a>
                )}
                {handicraft.instagramLink && (
                  <a
                    href={handicraft.instagramLink}
                    target="_blank"
                    className="flex items-center gap-2 px-4 py-2 bg-pink-50 text-pink-700 rounded-lg hover:bg-pink-100 text-sm"
                  >
                    <Instagram className="w-4 h-4" /> Instagram
                  </a>
                )}
              </div>
            )}
          </section>

          {/* MAP */}
          {hasCoords && (
            <section className="bg-white p-3 sm:p-4 rounded-2xl sm:rounded-[2.5rem] shadow-lg border border-amber-100 h-[300px] sm:h-[420px]">
              <DynamicMap
                defaultLat={lat}
                defaultLng={lng}
                city={handicraft.city}
                state={handicraft.state}
              />
            </section>
          )}

          {/* CONTRIBUTIONS */}
          {handicraft.contributions?.length > 0 && (
            <section className="bg-white p-6 sm:p-10 rounded-2xl sm:rounded-[2.5rem] shadow-lg border border-amber-100">
              <h2 className="text-2xl sm:text-3xl font-serif font-semibold text-amber-900 mb-6 relative inline-block">
                Visitor Contributions
                <div className="absolute -bottom-1 left-0 w-24 h-[2px] bg-amber-400 rounded-full" />
              </h2>

              <div className="space-y-6 sm:space-y-8">
                {handicraft.contributions.map((c) => (
                  <div
                    key={c._id}
                    className="border border-amber-100 rounded-xl p-4 sm:p-6 shadow-sm bg-amber-50/50"
                  >
                    {/* USER */}
                    <div className="flex items-center gap-3 mb-3">
                      <Avatar className="w-10 h-10 sm:w-12 sm:h-12 border border-amber-200">
                        <AvatarImage src={c.userId?.image} alt={c.userId?.username} />
                        <AvatarFallback className="bg-gradient-to-br from-amber-400 to-orange-500 text-white font-semibold">
                          {c.userId?.username?.[0]?.toUpperCase() || "A"}
                        </AvatarFallback>
                      </Avatar>

                      <div>
                        <p className="font-semibold text-amber-900 text-sm sm:text-base">
                          {c.userId?.username || "Anonymous user"}
                        </p>
                        <p className="text-xs sm:text-sm text-gray-600">
                          {new Date(c.createdAt).toLocaleDateString()}
                        </p>
                      </div>
                    </div>

                    {/* TEXT */}
                    <p className="text-gray-700 whitespace-pre-line text-sm sm:text-base mb-3">
                      {c.content}
                    </p>

                    {/* IMAGES */}
                    {c.images?.length > 0 && (
                      <div className="grid grid-cols-2 sm:grid-cols-3 gap-3 mt-2">
                        {c.images.map((img, index) => (
                          <img
                            key={index}
                            src={img}
                            className="rounded-xl shadow-md h-32 sm:h-40 w-full object-cover"
                          />
                        ))}
                      </div>
                    )}

                    {/* VIDEO */}
                    {c.videoLink && (
                      <a
                        href={c.videoLink}
                        className="inline-flex items-center gap-1 text-red-600 mt-3 text-sm hover:underline"
                        target="_blank"
                      >
                        🎬 Watch Video
                      </a>
                    )}

                    {/* INSTAGRAM */}
                    {c.instagramLink && (
                      <a
                        href={c.instagramLink}
                        className="block mt-2 text-pink-600 text-sm hover:underline"
                        target="_blank"
                      >
                        📸 View on Instagram
                      </a>
                    )}

                    {/* Suggested Changes */}
                    {c.suggestedChanges && (
                      <div className="mt-4 bg-white p-3 rounded-xl border border-amber-200">
                        <p className="font-semibold text-amber-900 mb-2">Suggested Updates</p>
                        <p className="text-gray-700 text-sm">
                          <strong>Price Range:</strong> {c.suggestedChanges.priceRange || "—"}
                        </p>
                        {Array.isArray(c.suggestedChanges.localMarkets) && (
                          <p className="text-gray-700 text-sm">
                            <strong>Local Markets:</strong>{" "}
                            {c.suggestedChanges.localMarkets.map(m => m.name).join(", ") || "—"}
                          </p>
                        )}
                      </div>
                    )}
                  </div>
                ))}
              </div>
            </section>
          )}
        </div>

        {/* RIGHT COLUMN */}
        <div className="space-y-6 sm:space-y-8 lg:sticky lg:top-24 h-fit">

          {/* PRIMARY MARKET */}
          {primaryMarket && (
            <div className="bg-gradient-to-br from-amber-500 to-orange-600 p-6 sm:p-10 rounded-2xl sm:rounded-[2.5rem] text-white shadow-lg">
              

              <a
                href={googleMapsUrl}
                target="_blank"
                className="flex items-center justify-center gap-2 w-full bg-white text-amber-800 font-semibold py-3 sm:py-4 rounded-xl sm:rounded-2xl hover:bg-amber-50 transition"
              >
                Get Directions <ExternalLink className="w-4 h-4 sm:w-5 sm:h-5" />
              </a>
            </div>
          )}

          {/* ALL MARKETS */}
          <div className="bg-white p-6 sm:p-10 rounded-2xl sm:rounded-[2.5rem] shadow-lg border border-amber-200">
            <h3 className="text-2xl font-serif font-semibold text-amber-900 mb-4 sm:mb-6">
              Available At
            </h3>

            <div className="space-y-4 sm:space-y-6">
              {handicraft.localMarkets?.map((market, i) => (
                <div key={i} className="border-b pb-4 last:border-none">
                  <p className="font-semibold text-amber-900 text-sm sm:text-base">
                    {market.name}
                  </p>
                  <p className="text-xs sm:text-sm text-gray-600">{market.address}</p>
                </div>
              ))}
            </div>
          </div>

        </div>
      </div>
    </div>
  );
}
